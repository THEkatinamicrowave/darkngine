...
import flixel.system.FlxMultiShader;
...
class FlxSprite extends FlxObject
{
	...
	**/
	...
	/**
	 * This is a custom shader MUAHAHAHAHAHAHHAHAHHAAAA
	 */
	public var shaders:FlxMultiShader;
	...
	override public function draw():Void
	{
		...
		if (shaders != null)
			shaders.setCamSize(_frame.frame.x, _frame.frame.y, _frame.frame.width, _frame.frame.height);
	...
	function drawSimple(camera:FlxCamera):Void
	{
		...
		var framePixels_shaded = framePixels;
		if (shadersEnabled && shaders != null)
			framePixels_shaded = shaders.renderAll(framePixels_shaded);

		camera.copyPixels(_frame, framePixels_shaded, _flashRect, _flashPoint, colorTransform, blend, antialiasing, null, wrapMode);
	}
	...
	function drawFrameComplex(frame:FlxFrame, camera:FlxCamera):Void
	{
		...
		if (layer != null)
			layer.drawPixels(this, camera, frame, framePixels_shaded, matrix, colorTransform, blend, antialiasing, null, wrapMode);
		else
			camera.drawPixels(frame, framePixels_shaded, matrix, colorTransform, blend, antialiasing, null, wrapMode);
	}
